...
struct MonthYearWheelPickerView: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> MonthYearWheelPicker {
        ...
        let minimumDate = Calendar.current.date(from: DateComponents(year: 2000, month: 1, day: 1)) ?? Date()
        picker.minimumDate = minimumDate
        picker.setDate(date, animated: false)
        picker.addTarget(context.coordinator, action: #selector(context.coordinator.dateChanged), for: .valueChanged)
        return picker
    }
    ...
    class Coordinator: NSObject {
        var parent: MonthYearWheelPickerView
        
        init(_ parent: MonthYearWheelPickerView) {
            self.parent = parent
        }
        
        @objc func dateChanged(_ pickerView: MonthYearWheelPicker) {
            let selectedMonth = pickerView.month
            let selectedYear = pickerView.year
            
            let calendar = Calendar.current
            
            if let nextMonthDate = calendar.date(byAdding: .month, value: 1, to: calendar.date(from: DateComponents(year: selectedYear, month: selectedMonth, day: 1))!) {
                if let lastDayOfMonth = calendar.date(byAdding: .day, value: -1, to: nextMonthDate) {
                    parent.date = lastDayOfMonth
                }
            }
        }
    }
