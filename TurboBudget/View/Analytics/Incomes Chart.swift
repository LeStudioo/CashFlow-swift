...
struct IncomesChosenMonthChart: View {
    ...
    var body: some View {
        if let account {
            ...
            if userDefaultsManager.isIncomeFromTransactionsChart && sortedTransactionsIncomeByDay.map({ $0.amount }).reduce(0, +) != 0 {
                VStack {
                    let monthOfSelectedDate = Calendar.current.dateComponents([.month, .year], from: filter.date)
                    NavigationLink(destination: { RecentIncomesView(account: $account, update: $update) }, label: {
                        VStack {
                            HStack {
                                VStack(alignment: .leading, spacing: 10) {
                                    if let month = monthOfSelectedDate.month {
                                        Text(NSLocalizedString("chart_incomes_incomes_in", comment: "") + " " + Calendar.current.monthSymbols[month - 1])
...
//MARK: - Preview
struct IncomesChosenMonthChart_Previews: PreviewProvider {
    
    @State static var updatePreview: Bool = false
    @State static var previewAccount: Account? = previewAccount1()
    
    static var previews: some View {
        IncomesChosenMonthChart(account: $previewAccount, update: $updatePreview)
    }
}
