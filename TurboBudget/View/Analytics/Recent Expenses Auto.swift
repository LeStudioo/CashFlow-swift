//
//  RecentExpensesAutomationsView.swift
//  CashFlow
//
//  Created by KaayZenn on 26/07/2023.
//
// Localizations 01/10/2023

import SwiftUI

struct RecentExpensesAutomationsView: View {
    
    //Custom type
    @Binding var account: Account?
    @ObservedObject var filter: Filter = sharedFilter
    @ObservedObject var userDefaultsManager = UserDefaultsManager.shared
    var catManager = PredefinedCategoryManager()
    ...
    //Enum
    @State private var filterTransactions: FilterForRecentTransaction = .expenses
    ...
    var getAllMonthForTransactions: [DateComponents] {
        ...
        if let account {
            for transaction in account.transactions.filter({ $0.comeFromAuto }) {
    ...
    var searchResults: [Transaction] {
        if let account {
            if searchText.isEmpty {
                if ascendingOrder {
                    return account.transactionsWithOnlyAutomations.filter { $0.amount < 0 }.sorted { $0.amount < $1.amount }.reversed()
                } else {
                    return account.transactionsWithOnlyAutomations.filter { $0.amount < 0 }.sorted { $0.amount < $1.amount }
                }
            } else {
                return account.transactionsWithOnlyAutomations.filter { $0.amount < 0 }.sorted { $0.amount < $1.amount }.filter { $0.title.localizedCaseInsensitiveContains(searchText) }
            }
        } else { return [] }
    }
    ...
    var body: some View {
        ...
        .padding(update ? 0 : 0)
        .background(Color.colorBackground.edgesIgnoringSafeArea(.all))
        .navigationTitle(NSLocalizedString("word_automations_expenses", comment: ""))
    ...
    func amountExpensesByMonth(month: Date) -> Double {
        if let account {
            return account.getAllExpensesTransactionsForChosenMonth(selectedDate: month).filter({ $0.comeFromAuto }).map({ $0.amount }).reduce(0, -)
        } else { return 0 }
    }
...
//MARK: - Preview
struct RecentExpensesAutomationsView_Previews: PreviewProvider {
    
    @State static var prevUpdate: Bool = false
    @State static var previewAccount: Account? = previewAccount1()
    
    static var previews: some View {
        RecentExpensesAutomationsView(account: $previewAccount, update: $prevUpdate)
    }
}
